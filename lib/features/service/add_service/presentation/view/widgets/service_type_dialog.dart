import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:service_app/features/service/add_service/data/models/service_type_model.dart';
import 'package:service_app/features/service/add_service/presentation/view_model/add_service_cubit.dart';
import 'package:service_app/l10n/app_localizations.dart';


class ServiceTypeDialog extends StatefulWidget{
  final Function(ServiceTypeResult serviceType) onServiceTypeSelected;
  const ServiceTypeDialog({super.key,required this.onServiceTypeSelected});

  @override
  State<ServiceTypeDialog> createState() => _ServiceTypeDialogState();
}

class _ServiceTypeDialogState extends State<ServiceTypeDialog> {
  bool isSearching = false;
  TextEditingController searchController = TextEditingController();
  @override
  Widget build(BuildContext context) {
    return AlertDialog(
      backgroundColor: Theme.of(context).colorScheme.onBackground,
      title: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          /* isSearching
              ? Expanded(
            child: TextFormField(
              controller: searchController,
              onChanged: (value) {
                if (searchController.text.isNotEmpty) {
                  context.read<AddServiceCubit>().getC(query: searchController.text);
                }
              },
              decoration: InputDecoration(
                hintText: "Search Status...",
                border: InputBorder.none,
              ),
            ),
          )
              : */
          Text(
          AppLocalizations.of(context)!.selectServiceType,
            style: TextStyle(fontSize: 20.sp, fontWeight: FontWeight.w600),
          ),
          /*IconButton(
            icon: Icon(isSearching ? Icons.close : Icons.search),
            onPressed: () {
              setState(() {
                isSearching = !isSearching;
                if (!isSearching) {
                  searchController.clear();
                  context.read<HomeCubit>().getStatus();
                }
              });
            },
          ),*/
        ],
      ),
      content: SizedBox(
        height: MediaQuery.of(context).size.height / 1.6,
        child: BlocBuilder<AddServiceCubit, AddServiceState>(
          builder: (context, state) {
            if (state is GetServiceTypeLoading) {
              return Center(
                child: CircularProgressIndicator(color: Colors.green),
              );
            } else if (state is GetServiceTypeError) {
              return Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.warning_amber_rounded, color: Colors.red, size: 50),
                    SizedBox(height: 12),
                    Text(
                      "An error occurred.",
                      style: TextStyle(color: Colors.red, fontSize: 18),
                    ),
                  ],
                ),
              );
            } else if (state is GetServiceTypeSuccess) {
              final serviceType =context.read<AddServiceCubit>().serviceTypeList;
              if (serviceType.isEmpty) {
                return Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Image.asset("assets/images/sheet.png",width:48.w,height: 48.h,),
                      SizedBox(height: 12.h),
                      Text(
                        "No Service Type Found",
                        style: TextStyle(fontSize: 20.sp, color: Colors.grey),
                      ),
                    ],
                  ),
                );
              }
              return ListView.separated(
                separatorBuilder: (context, index) => Divider(
                  color: Theme.of(context).colorScheme.primary,
                  thickness: 1,
                  endIndent: 8,
                  indent: 8,
                ),
                itemCount: serviceType.length,
                itemBuilder: (context, index) {
                  final serviceTypeItem = serviceType[index];
                  return ListTile(
                    leading: Image.asset("assets/images/sheet.png",width: 40.w,height: 40.h,),
                    title: Text(serviceTypeItem.name ?? "No Name",
                        style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.w500)),
                    onTap: () {
                      widget.onServiceTypeSelected(serviceTypeItem);
                      Navigator.pop(context);
                    },
                  );
                },
              );
            }
            return SizedBox(); // Default empty state
          },
        ),
      ),
    );
  }
}